...
def navigate_neural_twin(
    ...
    testbed.background_color = [1, 1, 1, 1.000]
    testbed.exposure = 0
    ...
    while True:
        ...
        render_mask = np.zeros_like(rendered_image)
        render_mask = np.where(
            rendered_image != np.array([255, 255, 255]), [255, 255, 255], [0, 0, 0]
        ).astype(np.uint8)

        # Show
        output_image = cv2.cvtColor(rendered_image, cv2.COLOR_RGB2BGR)
        cv2.imshow("rgb", output_image)
        cv2.imshow("mask", render_mask)
        wr(output_image)
